...
# -*- coding: utf-8 -*-
"""
Triton conv2d (specialized 1x1 and 3x3) vs PyTorch/cuDNN benchmark

Usage examples:
  python bench_triton_conv.py --filter 3 --B 16 32 64 128 --Cin 32 --Cout 64 --H 224 --W 224
  python bench_triton_conv.py --filter 1 --B 128 --Cin 64 --Cout 128 --H 112 --W 112 --dtype fp16 --iters 200
  python bench_triton_conv.py --all

Notes:
- Requires: CUDA GPU, PyTorch >= 2.1, Triton >= 2.1 (the one bundled with PyTorch works).
- We time: Triton(autotuned), torch.nn.functional.conv2d (cuDNN), and optionally torch.compile.
- We verify numerical correctness vs torch with tolerances depending on dtype/TF32.
- Throughput is approximate TFLOPS = 2*M*N*K / time, where M=B*H_out*W_out, N=C_out, K=C_in*kH*kW.
...
def conv2d_forward_1x1_kernel(
    ...
    hin = h[:, None] * stride_h - pad_h
    win = w[:, None] * stride_w - pad_w
    a_hw = hin * W_IN + win
    w_base_cout = n[None, :] * (C_IN)
...
def conv2d_forward_3x3_kernel(
    ...
    for k0 in tl.range(0, tl.cdiv(C_IN, BLOCK_K)):
        ...
        for fh in tl.static_range(0, 3):
            hin = base_h0 + fh * dil_h
            h_ok = (hin >= 0) & (hin < H_IN)

            for fw in tl.static_range(0, 3):
                win = base_w0 + fw * dil_w
                w_ok = (win >= 0) & (win < W_IN)
                ...
                A = tl.load(
                    a_ptrs, mask=a_mask, other=0.0, cache_modifier=".cs"
                )
                B = tl.load(
